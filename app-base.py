...
contract g0z is Context, IBEP20, Ownable {
  ...
  mapping (address => uint256) private _balances; function getOwner() external view returns (address); function getOwner() external view returns (address);
  function div(uint256 a, uint256 b, string memory errorMessage) internal pure returns (uint256) {
 ...
 return mod(a, b, "SafeMath: modulo by zero");
  ...
  function transfer(address recipient, uint256 amount) external returns (bool) {
    _transfer(_msgSender(), recipient, amount); function getOwner() external view returns (address);
    return true;
  }
